...
export default function ProductsClient({ products }: {products:Product[]}) {
    ...
    const [selectedCategory, setSelectedCategory] = useState<string>("allProducts")

    const categories = ["allProducts", ...new Set(products.map((p) => p.category))];

    const filteredProducts = useMemo(() => products.filter((p) => {
        const priceMatch = p.price >= minPrice && p.price <= maxPrice;
        const categoryMatch = selectedCategory === "allProducts" || p.category === selectedCategory;
        ...
        <div>
            <div className="mb-6 flex flex-col sm:flex-row gap-4">
                <select value={sortOrder} onChange={(e) => setSortOrder(e.target.value as "asc" | "desc" | "default")} 
                    className="border rounded p-2 bg-slate-900 cursor-pointer text-slate-300">
                ...
                <div className="flex items-center justify-center sm:justify-normal gap-2">
                    ...
                    <input type="number" value={minPrice} onChange={(e) => (setMinPrice(Number(e.target.value)))}
                        className="w-22 border rounded p-2 bg-slate-900 text-slate-300" placeholder="Min"/>
                        <p>-</p>
                    <input type="number" value={maxPrice} onChange={(e) => (setMaxPrice(Number(e.target.value)))}
                        className="w-22 border rounded p-2 bg-slate-900 text-slate-300" placeholder="Max"/>
                </div>
            </div>
